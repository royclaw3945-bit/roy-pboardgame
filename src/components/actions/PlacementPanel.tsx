    ...
    <div>
      ...
      {subPhase === 'PLACING' && char && (
        char.location === 'THEATER' ? (
          <TheaterWeekdaySelector player={player} mag={mag} char={char} charIdx={charIdx} state={state} dispatch={dispatchAction} />
        ) : (
          <SlotSelector player={player} mag={mag} char={char} charIdx={charIdx} state={state}
            onPlace={(slotIndex) => dispatchAction({ type: 'PLACE_CHARACTER', playerId: player.id, characterIdx: charIdx, slotIndex })}
          />
        )
      )}

      {subPhase === 'ACTING' && char && (
        <ActionPhase player={player} mag={mag} char={char} charIdx={charIdx} state={state} dispatch={dispatchAction} />
      )}
    </div>
  );
}
...
function CharInfoBar({ player, mag, char, ap }: { player: any; mag: any; char: any; ap?: number }) {
  return (
...
function TheaterWeekdaySelector({ player, mag, char, charIdx, state, dispatch }: {
  player: any; mag: any; char: any; charIdx: number; state: any;
...
function ActionPhase({ player, mag, char, charIdx, state, dispatch }: {
  ...
  return (
    <div className="action-group">
      <CharInfoBar player={player} mag={mag} char={char} ap={ap} />
      ...
      {ap > 0 && (
        <LocationActions location={location} player={player} state={state} dispatch={dispatch} />
      )}
