...
public static class ReflectionExtensions
{
	...
	{
		Enum obj = Enum.GetValues(type).Cast<Enum>().FirstOrDefault((Enum v) => v.ToString().GetNameVariants(culture).Contains(value, StringComparer.Create(culture, ignoreCase: true)));
		if (obj == null)
		{
			object obj2 = Convert.ChangeType(value, Enum.GetUnderlyingType(type), culture);
			if (obj2 != null && Enum.IsDefined(type, obj2))
			{
				obj = (Enum)Enum.ToObject(type, obj2);
			}
		}
		return obj;
	}
