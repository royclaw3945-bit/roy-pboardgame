...
[AddComponentMenu("UI/Zoom Scroll Rect", 37)]
[SelectionBase]
[ExecuteInEditMode]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public class ZoomScrollRect : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, ILayoutController
{
	...
	private readonly float m_PinchZoomSpeed = 0.0005f;

	[SerializeField]
	private float m_ScrollZoomSpeed = 0.005f;

	public float MinContentScaleFactor = 0.1f;

	public float MaxContentScaleFactor = 0.3f;
	...
	private Sequence scrollbarFadeSequenceVertical;

	private Sequence scrollbarFadeSequenceHorizontal;
	...
	{
		SetVisibilityOfScrollBars();
	}

	private void SetVisibilityOfScrollBars()
	{
		if (horizontalScrollbarVisibility == ScrollbarVisibility.PlatformSet)
		{
			horizontalScrollbarVisibility = ScrollbarVisibility.Permanent;
		}
		else if (horizontalScrollbarVisibility == ScrollbarVisibility.Hidden)
		{
			horizontalScrollbar.gameObject.SetActive(value: false);
		}
		else if (horizontalScrollbarVisibility != ScrollbarVisibility.Permanent)
		{
			horizontalScrollbar.onValueChanged.AddListener(delegate
			{
				FadeScrollBar(horizontalScrollbar, scrollbarFadeSequenceHorizontal);
			});
		}
		if (verticalScrollbarVisibility == ScrollbarVisibility.PlatformSet)
		{
			verticalScrollbarVisibility = ScrollbarVisibility.Permanent;
		}
		else if (verticalScrollbarVisibility == ScrollbarVisibility.Hidden)
		{
			verticalScrollbar.gameObject.SetActive(value: false);
		}
		else if (verticalScrollbarVisibility != ScrollbarVisibility.Permanent)
		{
			verticalScrollbar.onValueChanged.AddListener(delegate
			{
				FadeScrollBar(verticalScrollbar, scrollbarFadeSequenceVertical);
			});
		}
	}

	protected ZoomScrollRect()
	{
	}
	...
	public virtual void OnScroll(PointerEventData data)
	{
		...
		if (scrollMode == ScrollMode.Movement)
		{
	...
	private void ScrollZoom(PointerEventData data)
	{
		...
		m_Content.localScale += Vector3.one * y * m_ScrollZoomSpeed;
		m_Content.localScale = new Vector3(Mathf.Clamp(m_Content.localScale.x, MinContentScaleFactor, MaxContentScaleFactor), Mathf.Clamp(m_Content.localScale.y, MinContentScaleFactor, MaxContentScaleFactor), Mathf.Clamp(m_Content.localScale.z, MinContentScaleFactor, MaxContentScaleFactor));
	}

	private void ScrollMovement(PointerEventData data)
	{
		EnsureLayoutHasRebuilt();
		UpdateBounds();
		Vector2 scrollDelta = data.scrollDelta;
	...
	{
		if (eventData.button != PointerEventData.InputButton.Left || !IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, out var localPoint) || isBlocked)
		{
			return;
		}
		UpdateBounds();
	...
	private float GetDeltaPitch()
	{
		...
		if (Input.touchCount == 2)
		{
			...
			result = (magnitude - magnitude2) * -1f;
		}
		return result;
	}
	...
	private void HandlePinchZoom()
	{
		if (!isBlocked)
		{
			...
			m_Content.localScale += Vector3.one * deltaPitch * m_PinchZoomSpeed;
			m_Content.localScale = new Vector3(Mathf.Max(m_Content.localScale.x, MinContentScaleFactor), Mathf.Max(m_Content.localScale.y, MinContentScaleFactor), Mathf.Max(m_Content.localScale.z, MinContentScaleFactor));
			m_Content.localScale = new Vector3(Mathf.Min(m_Content.localScale.x, MaxContentScaleFactor), Mathf.Min(m_Content.localScale.y, MaxContentScaleFactor), Mathf.Min(m_Content.localScale.z, MaxContentScaleFactor));
		}
	}
	...
	private static void UpdateOneScrollbarVisibility(bool xScrollingNeeded, bool xAxisEnabled, ScrollbarVisibility scrollbarVisibility, Scrollbar scrollbar)
	{
		...
		else if (scrollbarVisibility == ScrollbarVisibility.Permanent)
		{
	...
	private void FadeScrollBar(Scrollbar scrollbar, Sequence scrollbarFadeSequence)
	{
		scrollbarFadeSequence.Pause();
		scrollbar.GetComponent<CanvasGroup>().DOFade(1f, UIProperties.AnimationDuration);
		scrollbarFadeSequence = DOTween.Sequence();
		scrollbarFadeSequence.AppendInterval(0.3f);
		scrollbarFadeSequence.Append(scrollbar.GetComponent<CanvasGroup>().DOFade(0f, UIProperties.AnimationDuration));
	}
	...
	public Vector2 CalculateFutureOffset(Bounds futureViewBounds, Vector2 delta)
	{
		if (m_ContentBounds.size.magnitude == 0f)
		{
			UpdateBounds();
		}
		return InternalCalculateOffset(ref futureViewBounds, ref m_ContentBounds, m_Horizontal, m_Vertical, m_MovementType, ref delta);
	}
